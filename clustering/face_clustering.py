...
'''
Modified section to save all clusters


'''
for i in range(0, num_classes):
    if not os.path.isdir(output_folder_path+"/" + str(i)):
        os.makedirs(output_folder_path+"/" + str(i))


print("Saving faces in all clusters to output folder...")
print(labels)
for i, label in enumerate(labels):
    print(label)
    img, shape = images[i]
    file_path = os.path.join(output_folder_path +"/" + str(label), "face_" + str(i))
    # The size and padding arguments are optional with default size=180x180 and padding=0.25
    dlib.save_face_chip(img, shape, file_path, size=180, padding=0.25)
